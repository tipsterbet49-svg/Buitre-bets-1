//#region node_modules/.nitro/vite/services/ssr/assets/crests-3Ve7QrE9.js
...
function normTeam(name) {
	return name.normalize("NFD").replace(/\p{M}/gu, "").replace(/[øØ]/g, "o").replace(/[æÆ]/g, "ae").replace(/[åÅ]/g, "a").toLowerCase().replace(/['’`]/g, "").replace(/&/g, " and ").replace(/[^a-z0-9]+/g, " ").replace(/\b(fc|cf|ssc|sk|ac|as|sc|cd|ud|rc|rcd|afc|cfc|bk|fk|if|the|club|1)\b/g, " ").replace(/\s+/g, " ").trim();
}
...
async function hydrateCrests(names) {
...
async function lookupOne(name) {
	...
	if (!k) return;
	try {
		const res = await fetch(`https://www.thesportsdb.com/api/v1/json/3/searchteams.php?t=${encodeURIComponent(name)}`, { signal: AbortSignal.timeout(4e3) });
		if (!res.ok) return;
		const teams = (await res.json()).teams ?? [];
		const hit = teams.filter((t) => (t.strSport ?? "").toLowerCase() === "soccer")[0] ?? teams[0];
		if (!hit) return;
		const api = Number(hit.idAPIfootball);
		const url = Number.isFinite(api) && api > 0 ? media(api) : hit.strBadge || void 0;
		if (url) runtime.set(k, url);
	} catch {}
}
//#endregion
export { isWeekend as a, hydrateCrests as i, crestUrl as n, dayBucket as r, artToday as t };
